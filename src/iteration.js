...
export function listItem(tasks) {
  ...
  for (let i = 0; i < tasks.length; i += 1) {
    list.innerHTML += `<li class='list-item'><input id='${tasks[i].index}' class='check' type="checkbox" ${tasks[i].completed?'checked':''} ><input class ='edit' placeholder = '${tasks[i].description}' id ='${tasks[i].index}' disabled> <button class = "button"id ='${tasks[i].index}'>remove</button></li>`;
  }
